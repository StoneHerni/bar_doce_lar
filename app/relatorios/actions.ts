...
export async function registerMovement(tipo: string, descricao: string, valor: number) {
  ...
  try {
    await run('INSERT INTO caixa (data, descricao, tipo, valor) VALUES (?, ?, ?, ?)', [today, descricao, tipo, valor]);
  ...
  try {
    ...
    for (const p of products) {
      ...
      const fechoItem = await get(`
        ...
        LIMIT 1
      `, [p.id, date]) as { quantidade: number; quantidade_prateleira: number; quantidade_arca: number } | undefined;
